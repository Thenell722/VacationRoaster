...
export function HolidaysView({ holidays }: { holidays: Hol[] }) {
  const router = useRouter();
  const [createOpen, setCreateOpen] = useState(false);
  const [editTarget, setEditTarget] = useState<Hol | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Hol | null>(null);
  ...
  async function onCreate(values: HolidayInput) {
    startTransition(async () => {
      const res = await createHoliday(values);
      if (res.error) toast.error(res.error);
      else { toast.success('Holiday created'); createForm.reset(); setCreateOpen(false); router.refresh(); }
    });
  }

  async function onEdit(values: HolidayInput) {
    if (!editTarget) return;
    startTransition(async () => {
      const res = await updateHoliday(editTarget.id, values);
      if (res.error) toast.error(res.error);
      else { toast.success('Holiday updated'); setEditTarget(null); router.refresh(); }
    });
  }
  ...
  async function onDelete() {
    ...
    startTransition(async () => {
      ...
      if (res.error) toast.error(res.error);
      else { toast.success('Holiday deleted'); setDeleteTarget(null); router.refresh(); }
    });
  }

  function openEdit(h: Hol) {
    editForm.reset({ name: h.name, date: format(parseISO(h.date), 'yyyy-MM-dd') });
    setEditTarget(h);
  }
  ...
  return (
    <>
      <PageHeader
        title="Holidays"
        description="Manage company holidays. These are excluded from working-day calculations."
        actions={
      ...
      />
      {sorted.length === 0 ? (
        <EmptyState icon={PartyPopper} title="No holidays" description="Add company holidays to exclude them from working-day calculations." />
      ) : (
        <Card>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-muted/50 text-left text-xs uppercase tracking-wide text-muted-foreground">
                  <tr><th className="px-4 py-3 font-medium">Name</th><th className="px-4 py-3 font-medium">Date</th><th className="px-4 py-3 font-medium text-right">Actions</th></tr>
                </thead>
                    ...
                    <tr key={h.id} className="hover:bg-muted/30">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="sm" onClick={() => openEdit(h)}><Pencil className="mr-1 h-3.5 w-3.5" /> Edit</Button>
                          <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive" onClick={() => setDeleteTarget(h)}><Trash2 className="mr-1 h-3.5 w-3.5" /> Delete</Button>
                        </div>
                      </td>
                    </tr>
                  ))}
