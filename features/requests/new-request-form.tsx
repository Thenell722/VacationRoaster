...
export function NewRequestForm({ holidays }: { holidays: Holiday[] }) {
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<VacationRequestInput>({
  ...
  const conflictingHolidays = useMemo(() => {
    if (!startDate || !endDate) return [];
    return holidays.filter((h) => {
      const d = parseISO(h.date);
      return d >= parseISO(startDate) && d <= parseISO(endDate) && !isWeekend(d);
    });
  }, [startDate, endDate, holidays]);
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Choose your dates and optionally add a reason.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                {errors.startDate && <p className="text-xs text-destructive flex items-center gap-1"><AlertCircle className="h-3 w-3" /> {errors.startDate.message}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor="endDate">End Date</Label>
                <Input id="endDate" type="date" min={startDate || today} {...register('endDate')} />
                {errors.endDate && <p className="text-xs text-destructive flex items-center gap-1"><AlertCircle className="h-3 w-3" /> {errors.endDate.message}</p>}
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="reason">Reason (optional)</Label>
              <Textarea id="reason" placeholder="e.g. Family vacation, medical appointment..." rows={3} {...register('reason')} />
            ...
            {workingDays !== null && (
              <div className="rounded-lg border bg-blue-50/50 p-4 dark:bg-blue-950/30">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-semibold text-blue-600 dark:text-blue-400">{workingDays}</span>
                </div>
                ...
                {conflictingHolidays.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {conflictingHolidays.map((h) => (
                      <span key={h.id} className="rounded-md bg-blue-100 px-2 py-0.5 text-xs text-blue-700 dark:bg-blue-900 dark:text-blue-200">{h.name} ({format(parseISO(h.date), 'MMM d')})</span>
                    ))}
                  </div>
                )}
              </div>
